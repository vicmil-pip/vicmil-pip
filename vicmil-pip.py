...
The idea is for all the packages to be cross-platform(windows, linux, mac), so the thing
    ...
    about specific packages at: https://vicmil.uk/pip

You can also opt-out of using vicmil-pip but still use the packages by navigating to
    https://vicmil.uk/pip, there you will find install instructions for installing
...
def install_installer():
    ...
    with urllib.request.urlopen('https://raw.githubusercontent.com/vicmil-pip/vicmil-pip/refs/heads/main/vicmil_pip/installer.py') as f:
...
"""
        ...
        if len(arguments) > 2 and arguments[1] == "-r":
            ...
            try:
                with open(filename, "r") as file:
                    for line in file:
                        vicmil_pip.installer.install_package(line.strip(), debug=False)
            ...
            vicmil_pip.installer.install_package(arguments[1])
    ...
    if arguments[0] == "force-install":
        ...
        if len(arguments) > 1:
            vicmil_pip.installer.remove_package(arguments[1])
            vicmil_pip.installer.install_package(arguments[1])
    ...
    if arguments[0] == "list":
        ...
        vicmil_pip.installer.print_installed_packages()
    ...
    if arguments[0] == "packages":
        ...
        vicmil_pip.installer.print_all_packages()
